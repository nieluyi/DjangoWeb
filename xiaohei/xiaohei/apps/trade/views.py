...
'''
1 问题 前端怎么组织post的参数，后端creat怎么获取post的参数进行保存
 后端serializers获取post参数，是包含在validated_data里面，可以用处理validated_data的方法处理参数
 传入参数是直接出现在序列化类中的，它对传入的参数进行了限制
'''
class ShoppingCartViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        print('查看view数据',list(shop_carts))
...
'''
该视图为创建订单信息的view 
    post参数为 consignee_addr' 'order_price' 'goods'数组（只用id和num）
    返回数据  'order_id' 'user_id' 'order_number' 'order_price'  'consignee_addr' 'pay_status'
...
class AliPayView(mixins.CreateModelMixin,viewsets.GenericViewSet):
    ...
    def create(self, request, *args, **kwargs):
      if request.method == 'POST':
        ...
        request_data = json.loads(request.body.decode())#.replace("'", "\""))[0]

        user=request.user.id


        addr = request_data.get('consignee_addr')
        order_price = request_data.get('order_price')
        goods = request_data.get('goods')


        # 订单id: 20171122181630+用户id
        order_id = datetime.now().strftime('%Y%m%d%H%M%S') + str(user)
        # 获取时间
        ...
        for sku_id in goods:
            ...
            print(sku_id)
            print(type(sku_id))
            sku = OrderGoods.objects.get(id=sku_id['goods_id'])
            skus.append(sku)

        # UserProfile.objects.filter(openid=openid):

        OrderInfo.objects.create(
                user_id = user,            # 用户id
                order_sn=order_id,         # 订单编号
                address=addr,              # 收货地址
                order_amount=order_price,  # 订单价格
                add_time=creat_time,       # 下单时间
            )

        order =  OrderInfo.objects.get(order_sn=order_id)
        # order2 = OrderInfo.objects.filter(order_sn=order_id)
        # print(order2[0])
        for good in goods:
            OrderGoods.objects.create(
                goods_nums = good['goods_number'],
                add_time   = creat_time,
...
from xiaohei.settings  import app_id, alipay_debug, alipay_public_key_path, app_private_key_path
from django.utils import timezone
from django.shortcuts import redirect, reverse



class AliPayView2(APIView):
